...
DEPLOY_FILES = [
    ...
    "restart_bots.sh",
    "deploy_providers.sh",
    "os_orchestrator/lib/process_control.sh",
    "os_orchestrator/livecheck/vpn_watchdog.sh",
    "os_orchestrator/livecheck/vpn_watchdog.sh",
...
def sandbox(tmp_path):
    # deploy_providers.sh, restart_bots.sh, os_orchestrator/lib/process_control.sh, env_common.sh
    for name in ("os_orchestrator/admin/deploy_providers.sh", "os_orchestrator/admin/restart_bots.sh", "os_orchestrator/lib/env_common.sh"):
        shutil.copy2(ROOT / name, tmp_path / Path(name).name)
    (tmp_path / "os_orchestrator" / "lib").mkdir(parents=True, exist_ok=True)
    shutil.copy2(ROOT / "os_orchestrator/lib/process_control.sh", tmp_path / "os_orchestrator/lib/process_control.sh")
...
def test_successful_deploy_refreshes_both_roles_before_reporting_success(tmp_path):
    ...
    (tmp_path / "os_orchestrator/lib/process_control.sh").write_text('''
    ...
    try:
        found = subprocess.check_output(
            ["bash", "-c", 'source "$1"; manifest_pids registry_test_bot.py "$2"',
             "test", str(ROOT / "os_orchestrator/lib/process_control.sh"), str(first)], text=True, timeout=10)
        ...
        stopped = subprocess.run(
            ["bash", "-c", 'source "$1"; stop_manifest_process registry_test_bot.py "$2"',
             "test", str(ROOT / "os_orchestrator/lib/process_control.sh"), str(first)], timeout=15)
...
def _run_healthcheck(tmp_path, failing=None, mode="--check"):
    ...
    result = subprocess.run(
        ["bash", str(ROOT / "os_orchestrator/livecheck/vpn_watchdog.sh"), mode],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("os_orchestrator/livecheck/vpn_watchdog.sh", "PIA_CLI_TIMEOUT"),
        ("os_orchestrator/livecheck/vpn_watchdog.sh", "PIA_CLI_TIMEOUT"),
    ],
)
...
def test_alert_transports_reject_http_errors():
    assert "--fail-with-body" in _text("os_orchestrator/livecheck/deadman_switch.sh")
    assert "--fail-with-body" in _text("os_orchestrator/livecheck/vpn_watchdog.sh")
    assert "--fail-with-body" in _text("os_orchestrator/livecheck/vpn_watchdog.sh")


def test_installer_is_verified_with_pinned_sha256():
    text = _text("os_orchestrator/livecheck/vpn_watchdog.sh")
...
def test_healthcheck_surfaces_ntfy_http_failure(tmp_path):
    ...
    result = subprocess.run(
        ["bash", str(ROOT / "os_orchestrator/livecheck/vpn_watchdog.sh"), "--alert"],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
...
def test_autodeploy_is_shadow_by_default_and_never_reboots():
    text = _text("os_orchestrator/admin/manage_gitautodeploy.sh")
...
def test_deadman_has_an_independent_healthchecks_channel():
    ...
    text = _text("os_orchestrator/livecheck/deadman_switch.sh")
